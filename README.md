...
**Standalone smart keyboard (primary build)**: `BOARD_TYPE FAIRBERRY_ESP32S3_SMART` on an ESP32-S3. See [Hardware_Standalone_Smart_Keyboard.md](Documentation/Hardware_Standalone_Smart_Keyboard.md) for the pin plan, BOM (mic, speaker, microSD, LiPo + charger), firmware, and key combo table. Opt-in flags in `configuration.h`: `TRACKBALL_ENABLED`, `AUDIO_ENABLED` (local recording/playback, fully offline), `WIFI_TRANSCRIPTION_ENABLED` (syncs recordings to the OpenAI Whisper API), `TRANSFER_MODE_ENABLED` (local no-cloud web dashboard for browsing/downloading notes over LAN). The last two need `BBQ10/secrets.h` -- copy `secrets.h.example`. No enclosure design yet -- see that doc's Enclosure section for why. **None of this is validated against real hardware.**
