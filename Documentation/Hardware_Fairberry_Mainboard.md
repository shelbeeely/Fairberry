...
- `ESP32`: presents as a **Bluetooth LE HID** keyboard, no cable needed. This is the recommended path for Free Ink SDK devices — the SDK has an actual BLE HID host library for this, [`BleKeyboardHost`/`lib-ble`](https://freeink.org/docs/lib-ble), which explicitly supports external keyboards (not just page-turner remotes) on both ESP32-C3 and ESP32-S3. It's gated behind the `FREEINK_CAP_BLE_HID_HOST` build flag though, which is off by default — so the firmware actually flashed on your reader needs to have that capability turned on. See the README's Status section for where this stands.
- `FAIRBERRY_V0_3_0` (or another custom-mainboard version) / `ARDUINO`: presents as a standard **USB HID** keyboard over the OTG cable instead. Whether your reader accepts this depends on whether its firmware runs USB in host mode — I haven't found documented USB HID host support in Free Ink SDK for any target, so treat this path as more speculative than the BLE one.
...
- If you are using an Xteink X4, keep `include <presets/XteinkX4.scad>` uncommented (this is the default in this fork). Read the comment at the top of that file first — it's built from a community-measured device model rather than just the raw spec sheet, and includes steps for swapping in the actual dummy model for a better fit, but hasn't been validated against a physical unit. Treat the first print as a test fit.
